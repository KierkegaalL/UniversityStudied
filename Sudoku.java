...
public class Sudoku extends JFrame implements ActionListener {
	...
	private void prepareSudokuUI() {
		...
		JPanel buttonsPanel3 = new JPanel();
                ...
                buttonsPanel3.add(inverseButton);
	...
	public void actionPerformed(ActionEvent event) {
		...
		String buttonType = button.getText();
            ...
            switch (buttonType) {
                ...
                case "Inverse":
                    algorithmNum=1;
                    System.out.println("現在のアルゴリズム：Inverse");
                    break;
                case "Advanced":
                    algorithmNum=2;
	...
	private boolean bruteForceSolve(int row, int col) {
        ...
        */
        ...
        /**
     * Inverse Algorithm(Inverse)
         * 総当たり法のアルゴリズムを逆から処理していく
         * 基本的には総当たり法とは形は大差ないが、右下から進むように改造されている
	 * @return 解が出た場合true、解がでない場合falseを返す.
	 */
        private boolean InverseSolve(int row, int col) { 
            //全てのセルへの入力が終了している場合にメソッドを閉じていく
            if (row == -1)
                return true;

            // 注目したセルへ既に数字が入力されている場合に処理を次のセルへ移動する
            if (!(checkEmptyCell(row,(col%9)))) {         
                if (InverseSolve((col%9) == 0 ? (row - 1): row, (col - 1) ))
                    return true;
            } else {
                Integer[] randoms = generateRandomNumbers();
                for (int i = 8; i >= 0; i--) {
                    // 行、列、3x3ボックス内に数字が重複しない場合に値を代入し、次のセルへ進む
                    if (!isContainedInRowColumn(row, (col%9), randoms[i]) &&
                                    !isContainedIn3x3Box(row, (col%9), randoms[i])) {
                        setCellValues(row, (col%9), randoms[i]);
			// 次のマス（総当たり法の逆なのでの右→左、下→上という順）に進む
                        if(InverseSolve((col%9) == 0 ? (row - 1):row,(col - 1)))
                            return true;
                         else { // 次のマスに入る数字がなく、このマスに別の数字を入れないといけないので、一度入れた数字を初期化する
                            setCellValues(row,(col%9),0);
                        }
                    }
                }
            }
